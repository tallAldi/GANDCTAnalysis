...
def _gather_paths(directory: str, per_dir_limit: Optional[int]) -> List[str]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--csv_out", "-o", type=str, default=None, help="Output CSV dir.")
...
def main() -> None:
    ...
    trained_on = args.MODEL.split('\\')[-2]

    assert trained_on in ['celebA', 'lsun'], f"Model path does not indicate correct dataset (celebA or lsun): {trained_on}" 
    ...
    for dir in args.DATA:
        paths = _gather_paths(dir, args.size)
        total = len(paths)
        assert total >= 1
        ...
        if csv_out is None:
            csv_out = os.path.join(os.path.abspath(dir), f"{trained_on}_predictions.csv")
        else:
            csv_out = os.path.join(os.path.abspath(csv_out), f"{dir.split(os.sep)[-3]}_{dir.split(os.sep)[-2]}_{trained_on}_predictions.csv")
        ...
        with open(csv_out, mode="w", newline="", encoding="utf-8") as f_csv:
            ...
            for s in tqdm(range(steps), desc=f"{dir.split(os.sep)[-3]}/{dir.split(os.sep)[-2]}", unit="batch"):
                start = s * batch_size
